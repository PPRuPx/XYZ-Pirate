...
namespace Model
{
    public class StatsModel : IDisposable
    {
        ...
        public float GetValue(StatId id, int level = -1) =>
            GetStatLevelDef(id, level).Value;

        public StatLevelDef GetStatLevelDef(StatId id, int level = -1)
        {
        ...
        public int GetCurrentLevel(StatId id) => 
            _data.Levels.GetLevel(id);
        
        public bool CanUpgrade(StatId id, int level = -1)
        {
            var def = GetStatLevelDef(id, level);
